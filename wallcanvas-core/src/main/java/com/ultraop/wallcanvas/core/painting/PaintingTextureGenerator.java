...
public final class PaintingTextureGenerator {
    ...
    public static Path generate(Path source, Path packRoot, PaintingVariantDefinition definition) throws IOException {
        if (!Files.isRegularFile(source)) throw new IOException("Picture does not exist: " + source);

        BufferedImage input = ImageIO.read(source.toFile());
        if (input == null) throw new IOException("Unsupported or unreadable image format: " + source.getFileName());

        int targetWidth = definition.width() * definition.pixelsPerBlock();
        int targetHeight = definition.height() * definition.pixelsPerBlock();
        ...
        try {
            ...
            graphics.drawImage(input, 0, 0, targetWidth, targetHeight,
        ...
        Path texture = PaintingResourcePackLayout.texture(packRoot, definition.variantId())
                .resolveSibling(PaintingResourcePackLayout.texture(packRoot, definition.variantId()).getFileName().toString()
                        .replace(".json", ".png"));
        Files.createDirectories(texture.getParent());
        if (!ImageIO.write(output, "PNG", texture.toFile())) throw new IOException("PNG encoder is unavailable");
        return texture;
    }
}
